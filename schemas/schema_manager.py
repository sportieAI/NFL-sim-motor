...
class SchemaManager:
    ...
    def _load_schemas(self):
        ...
        for schema_file in self.storage_path.glob("*.json"):
            try:
                ...
                self.schemas[schema_version.schema_name][str(schema_version.version)] = schema_version
                ...
                if not schema_version.deprecated:
                    ...
                    if not current or schema_version.version > current:
                        self.current_versions[schema_version.schema_name] = schema_version.version
    ...
    def _initialize_default_schemas(self):
        ...
        game_state_v1 = {
            ...
            "required": ["down", "distance", "field_position", "quarter", "time_remaining"],
            "properties": {
                "down": {
                    "type": "integer",
                    "minimum": 1,
        ...
        play_result_v1 = {
            ...
            "properties": {
                ...
                "play_type": {
                    "type": "string",
                    "enum": ["run", "pass_short", "pass_medium", "pass_deep", "punt", "field_goal", "extra_point"],
    ...
    def register_schema(self, schema_name: str, version: SemanticVersion, schema: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to register schema {schema_name} v{version}: {e}")
            return False
    
    def validate(self, data: Dict[str, Any], schema_name: str, version: Optional[str] = None) -> tuple[bool, Optional[str]]:
    ...
    def deprecate_schema_version(self, schema_name: str, version: str, replacement_version: Optional[str] = None) -> bool:
    ...
    def get_migration_instructions(self, schema_name: str, from_version: str, to_version: str) -> List[Dict[str, Any]]:
        """Get migration instructions between schema versions."""
        if (schema_name not in self.schemas or 
            from_version not in self.schemas[schema_name] or
            to_version not in self.schemas[schema_name]):
            return []
    ...
    def get_all_schemas(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for schema_name, versions in self.schemas.items():
            result[schema_name] = []
            for version_str, schema_version in sorted(versions.items(), 
                ...
                result[schema_name].append({
                    ...
                    "is_current": version_str == str(self.current_versions.get(schema_name))
                })
    ...
    def _save_schema(self, schema_version: SchemaVersion):
        ...
        try:
            ...
            self.logger.error(f"Failed to save schema {schema_version.schema_name} v{schema_version.version}: {e}")
...
class SchemaTestRunner:
    ...
    def _test_schema(self, schema_name: str) -> Dict[str, Any]:
        ...
        for test_case in test_cases:
            is_valid, error = self.schema_manager.validate(test_case["data"], schema_name)
    ...
    def _generate_test_cases(self, schema_name: str, schema: Dict[str, Any]) -> List[Dict[str, Any]]:
