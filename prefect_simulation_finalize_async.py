...
async def fetch_final_events(sim_id: str) -> List[Dict]:
    ...
    return [{"event_id": 1, "type": "score", "details": "TD"}, {"event_id": 2, "type": "penalty", "details": "Offside"}]
...
async def load_memories(sim_id: str) -> List[Dict]:
    await asyncio.sleep(0.1)
    return [{"event_id": 1, "summary": "TD by Player 7"}, {"event_id": 2, "summary": "Offside"}]

async def build_narrative_prompt(memories: List[Dict]) -> str:
    return "Summarize the following events: " + "; ".join([m["summary"] for m in memories])
...
@task(name="Terminate Simulation", retries=2, retry_delay_seconds=3, persist_result=False)
async def terminate_simulation(sim_id: str):
    await terminate_simulation_api(sim_id)
    await persist_final_state(sim_id)

@task(name="Persist Final Events and Memories", retries=2, retry_delay_seconds=3, persist_result=False)
