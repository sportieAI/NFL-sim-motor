...
class SimulationState:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            1 <= self.down <= 4
            and 1 <= self.distance <= 99
            and 0 <= self.field_position <= 100
            and 1 <= self.quarter <= 4
            and 0 <= self.time_remaining <= 3600
            and -100 <= self.score_differential <= 100
            and 0 <= self.timeouts_remaining <= 3
        )
...
class PlayAction:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            self.play_type
            in ["run", "pass_short", "pass_medium", "pass_deep", "punt", "field_goal"]
            and -10 <= self.target_yards <= 50
            and 0.0 <= self.risk_level <= 1.0
        )
...
class PlayResult:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            -20 <= self.yards_gained <= 100
            and self.time_elapsed >= 0
            and not (self.turnover and self.touchdown)  # Can't have both
        )
...
class SimulationPropertyTester:
    ...
    def __init__(self):
        ...
        if not HYPOTHESIS_AVAILABLE:
            self.logger.warning(
                "Hypothesis not available. Property-based testing disabled."
            )
    ...
    def test_game_situation_properties(self):
        ...
        def test_game_situation_validity(
            down, distance, field_position, quarter, time_remaining, score_differential
        ):
    ...
    def test_simulation_state_transitions(self):
        ...
        def test_state_transition_properties(initial_state, action, result):
            ...
            if not (
                initial_state.is_valid() and action.is_valid() and result.is_valid()
            ):
                return
    ...
    def _simulate_state_transition(
        self, state: SimulationState, action: PlayAction, result: PlayResult
    ) -> SimulationState:
        ...
        new_state.field_position = max(
            0, min(100, state.field_position + result.yards_gained)
        )
...
if HYPOTHESIS_AVAILABLE:
    ...
    class SimulationStateMachine(RuleBasedStateMachine):
        ...
        def execute_play(self, play_type, yards):
            ...
            self.current_state = tester._simulate_state_transition(
        ...
        def time_decreases_monotonically(self):
            ...
            if len(self.game_states) >= 2:
                assert (
                    self.game_states[-1].time_remaining
                    <= self.game_states[-2].time_remaining
                )
