...
class RedisBackend(StorageBackend):
    ...
    def __init__(self, config: StorageConfig):
        ...
        redis_url = config.redis_url or redis_settings.get("redis_url", "redis://localhost:6379/0")
        ...
        try:
            ...
            self.logger = logging.getLogger(__name__)
            self.logger.warning(f"Redis connection failed: {e}. Operating in degraded mode.")
            self.client = None
...
class PostgresBackend(StorageBackend):
    ...
    def __init__(self, config: StorageConfig):
        if not POSTGRES_AVAILABLE:
            raise ImportError("Postgres dependencies not available. Install with: pip install psycopg2-binary sqlalchemy")
        ...
        try:
            ...
            self.logger = logging.getLogger(__name__)
            self.logger.warning(f"Postgres connection failed: {e}. Operating without persistent storage.")
            self.engine = None
            self.session = None
    
    def store(self, key: str, value: Any, table: str = "simulation_logs", **kwargs) -> bool:
...
class VectorIndex:
    ...
    def add_vector(self, key: str, vector: np.ndarray, metadata: Dict[str, Any] = None) -> bool:
        """Add a vector to the index."""
        try:
            if vector.shape != (self.dimension,):
                self.logger.error(f"Vector dimension mismatch: expected {self.dimension}, got {vector.shape}")
                return False
    ...
    def search_similar(self, query_vector: np.ndarray, k: int = 10) -> List[Tuple[str, float, Dict[str, Any]]]:
        """Search for similar vectors."""
        try:
            if query_vector.shape != (self.dimension,):
                self.logger.error(f"Query vector dimension mismatch: expected {self.dimension}, got {query_vector.shape}")
                return []
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_vectors": len([v for v in self.id_map.values() if not v.get("deleted", False)])
        }
...
class UnifiedStorageManager:
    ...
    def store_with_vector(self, key: str, value: Any, vector: np.ndarray, 
                         metadata: Dict[str, Any] = None, store_cold: bool = True) -> bool:
    ...
    def search_similar_plays(self, query_vector: np.ndarray, k: int = 10) -> List[Tuple[str, float, Dict[str, Any]]]:
