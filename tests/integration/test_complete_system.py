...
from evaluation.policy_evaluator import PolicyEvaluator, PolicyConfig, PolicyType, DriveContext
...
from messaging.reliable_sender import ReliableMessageSender, HTTPTransport, MessagePriority
...
class IntegrationTestSuite:
    ...
    async def _test_simulation_robustness(self) -> Dict[str, Any]:
        ...
        try:
            ...
            orchestrator = SimulationOrchestrator(state_dim=10, action_dim=5, game_id="test-robustness")
            
            # Create realistic game situation
            game_situation = GameSituation(
                down=3, distance=7, field_position=65,
                quarter=4, time_remaining=300, score_differential=-3
            )
            ...
            orchestrator.run_simulation(initial_state, num_plays=5, game_situation=game_situation)
            ...
            print(f"   ✅ Top play type: {priors[0].play_type.value} ({priors[0].probability:.3f})")
    ...
    async def _test_policy_evaluation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for i in range(10):
                drive = DriveContext(
                    ...
                    starting_situation=GameSituation(
                        ...
                        quarter=1, time_remaining=3600, score_differential=0
                    ),
            ...
            return {
                ...
                "has_statistical_significance": len(result.statistical_significance) > 0
            }
    ...
    async def _test_feature_systems(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"   ✅ Cluster analysis: {cluster_analysis.get('total_clusterings', 0)} runs")
    ...
    async def _test_testing_framework(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"   ✅ Property tests: {'PASSED' if property_tests_passed else 'FAILED'}")
            print(f"   ✅ Fuzz tests: {fuzz_report['summary']['total_tests']} tests")
            print(f"   ✅ Fuzz success rate: {fuzz_report['summary']['success_rate']:.1f}%")
    ...
    async def _test_schema_management(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"   ✅ Schema tests: {'PASSED' if schema_results['overall_success'] else 'FAILED'}")
    ...
    def _print_test_summary(self):
        ...
        # Robustness metrics
        if "robustness" in self.test_results and self.test_results["robustness"]["success"]:
        ...
        if "policy_evaluation" in self.test_results and self.test_results["policy_evaluation"]["success"]:
        ...
        if "storage" in self.test_results and self.test_results["storage"]["success"]:
            ...
            print(f"  • Vector similarity search: {'Working' if stor['vector_storage_works'] else 'Failed'}")
        
        # Messaging metrics
        if "messaging" in self.test_results and self.test_results["messaging"]["success"]:
        ...
        print("   evaluation capabilities, storage integration, and comprehensive testing.")
