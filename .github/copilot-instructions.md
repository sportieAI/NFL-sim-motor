...
- **Broken test imports**: Tests reference old class names. Use `cluster_play` function from `clustering.py` and `MemoryContinuity` class from `memory_continuity.py`
- **Syntax errors**: Several files have unterminated strings and formatting issues causing Python syntax errors
...
- **Entry point**: `tests/test_full_simulation_suite.py` (working)
- **Dependencies**: `requirements.txt` (working version)
- **Tests**: `tests/` directory (has import issues to fix)
...
- `tests/test_full_simulation_suite.py` - Core simulation test (works)
