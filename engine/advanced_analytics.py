...
class AdvancedAnalytics:
    ...
    def player_efficiency(self, player_id):
        stats = [r.get("yards", 0) for s, r in self.events if r.get("player_id") == player_id]
        return round(np.mean(stats), 2) if stats else 0.0

    def drive_success_rate(self, team):
        drives = [r for s, r in self.events if r.get("team") == team and r.get("event") == "drive_end"]
        return round(np.mean([1 if d.get("result") == "TD" else 0 for d in drives]), 2) if drives else 0.0
