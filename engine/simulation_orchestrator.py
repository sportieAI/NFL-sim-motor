...
class SimulationOrchestrator:
    ...
    def run_simulation(
        ...
        for play_number in range(num_plays):
            ...
            play_context = PlayContext(
                ...
                quarter=(
                    getattr(state, "quarter", 1) if hasattr(state, "quarter") else 1
                ),
                down=getattr(state, "down", 1) if hasattr(state, "down") else 1,
                distance=(
                    getattr(state, "distance", 10) if hasattr(state, "distance") else 10
                ),
                field_position=(
                    getattr(state, "field_position", 50)
                    if hasattr(state, "field_position")
                    else 50
                ),
                team=(
                    getattr(state, "team", "HOME") if hasattr(state, "team") else "HOME"
                ),
            )

            try:
                # Execute play with robustness
                success = self._execute_play_with_robustness(
                    state, play_context, game_situation
                )

                if not success:
                    self.logger.warning(
                        f"Play {play_id} failed, continuing with degraded state"
                    )
                    continue
                ...
                if done:
                    self.logger.info(
                        f"Simulation completed early at play {play_number + 1}"
                    )
                    break
    ...
    def _execute_play_with_robustness(
        self,
        state,
        play_context: PlayContext,
        game_situation: Optional[GameSituation] = None,
        ...
        if sim_error:
            ...
            play_result = (
                ...
                0.0,
                False,
                {"summary": "Error during play execution", "tags": ["error"]},
            )
        ...
        commentary, comm_error = safe_execute_with_context(
            ...
            play_context,
            result.get("summary", "Play occurred"),
        )
        if comm_error:
    ...
    def _select_action_with_priors(
        self, state, game_situation: Optional[GameSituation] = None
    ) -> int:
        ...
        if game_situation:
            ...
            self.logger.debug(
                f"Play priors: {[(p.play_type.value, p.probability) for p in priors[:3]]}"
            )
    ...
    def simulate_play(self, state, action):
        ...
        if random.random() < 0.05:  # 5% chance of simulation error
            raise PlayExecutionError(
                "Simulated play execution failure", recoverable=True
            )
        ...
        result = {
            "summary": f"Action {action} taken.",
            "tags": ["aggressive" if action % 2 == 0 else "conservative"],
        }
