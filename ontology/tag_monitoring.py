...
class TagConfidenceManager:
    ...
    def __init__(
        self,
        storage_path: str = "logs/tag_analysis",
        ...
        self.calibration_history: Dict[str, List[ConfidenceCalibration]] = defaultdict(
            list
        )
    ...
    def record_tag_instance(
        self, tag_instance: TagInstance, ground_truth: Optional[bool] = None
    ):
        ...
        if tag_instance.tag_name not in current_tags:
            self.logger.warning(
                f"Tag {tag_instance.tag_name} not found in current ontology"
            )
            return

        # Validate confidence range
        if not self.ontology_manager.validate_tag_confidence(
            tag_instance.tag_name, tag_instance.confidence
        ):
            self.logger.warning(
                f"Confidence {tag_instance.confidence} outside expected range for {tag_instance.tag_name}"
            )
        ...
        if (
            len(self.tag_instances[tag_instance.tag_name])
            % self.calibration_update_frequency
            == 0
        ):
    ...
    def _update_calibration(self, tag_name: str):
        ...
        labeled_instances = [
            inst for inst in instances if inst.get("ground_truth") is not None
        ]
        ...
        if calibration.calibration_error > 0.2:
            self.logger.warning(
                f"High calibration error for {tag_name}: {calibration.calibration_error:.3f}"
            )
    ...
    def _compute_calibration_metrics(
        self, tag_name: str, instances: List[Dict]
        ...
        reliability_score = (
            np.corrcoef(confidences, ground_truths)[0, 1] if len(instances) > 1 else 0.0
        )
    ...
    def _update_drift_analysis(self, tag_name: str):
        ...
        drift_metrics = self._compute_drift_metrics(
            tag_name, baseline_instances, current_instances
        )
        ...
        if drift_metrics.drift_severity in ["medium", "high"]:
            self.logger.warning(
                f"Tag drift detected for {tag_name}: {drift_metrics.drift_severity} "
    ...
    def _compute_drift_metrics(
        ...
        baseline_time_span = (
            baseline_instances[-1]["timestamp"] - baseline_instances[0]["timestamp"]
        )
        current_time_span = (
            current_instances[-1]["timestamp"] - current_instances[0]["timestamp"]
        )

        baseline_freq = len(baseline_instances) / max(baseline_time_span, 1)
        current_freq = len(current_instances) / max(current_time_span, 1)
        usage_frequency_change = (current_freq - baseline_freq) / max(
            baseline_freq, 0.001
        )

        # Determine drift severity
        drift_severity = self._assess_drift_severity(
            js_divergence, abs(confidence_mean_shift), abs(usage_frequency_change)
        )
    ...
    def _assess_drift_severity(
        self, js_divergence: float, mean_shift: float, freq_change: float
    ) -> str:
        ...
        high_indicators = (
            js_divergence > high_js_threshold
            or mean_shift > high_mean_threshold
            or freq_change > high_freq_threshold
        )

        medium_indicators = (
            js_divergence > medium_js_threshold
            or mean_shift > medium_mean_threshold
            or freq_change > medium_freq_threshold
        )
    ...
    def generate_calibration_plot_data(self, tag_name: str) -> Optional[Dict[str, Any]]:
        ...
        if (
            tag_name not in self.calibration_history
            or not self.calibration_history[tag_name]
        ):
            return None
        ...
        predicted_probs = [
            bin_data[0] for bin_data in latest_calibration.confidence_bins
        ]
    ...
    def get_drift_summary(self, tag_name: str, days_back: int = 30) -> Dict[str, Any]:
        ...
        recent_drift = [
            d for d in self.drift_history[tag_name] if d.time_window_end > cutoff_time
        ]
        ...
        return {
            ...
            "high_severity_events": len(
                [d for d in recent_drift if d.drift_severity == "high"]
            ),
            "medium_severity_events": len(
                [d for d in recent_drift if d.drift_severity == "medium"]
            ),
            "avg_js_divergence": np.mean([d.js_divergence for d in recent_drift]),
            "avg_confidence_shift": np.mean(
                [d.confidence_mean_shift for d in recent_drift]
            ),
            "avg_frequency_change": np.mean(
                [d.usage_frequency_change for d in recent_drift]
            ),
    ...
    def get_all_tag_health(self) -> Dict[str, Dict[str, Any]]:
        ...
        for tag_name in self.tag_instances.keys():
            ...
            health_summary[tag_name] = {
                ...
                "drift": drift_summary,
                "last_updated": (
                    max([inst["timestamp"] for inst in self.tag_instances[tag_name]])
                    if self.tag_instances[tag_name]
                    else 0
                ),
            }
    ...
    def _save_calibration(self, calibration: ConfidenceCalibration):
        ...
        try:
            file_path = (
                self.storage_path
                / f"calibration_{calibration.tag_name}_{int(calibration.timestamp)}.json"
            )
            with open(file_path, "w") as f:
                json.dump(asdict(calibration), f, indent=2)
        except Exception as e:
            self.logger.error(
                f"Failed to save calibration for {calibration.tag_name}: {e}"
            )
    ...
    def _save_drift_metrics(self, drift: TagDriftMetrics):
        ...
        try:
            file_path = (
                self.storage_path
                / f"drift_{drift.tag_name}_{int(drift.time_window_end)}.json"
            )
            with open(file_path, "w") as f:
