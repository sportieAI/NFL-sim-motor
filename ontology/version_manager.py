...
class OntologyManager:
        ...
        for file_path in self.storage_path.glob("*.json"):
            try:
                ...
                version_obj = OntologyVersion(
                    ...
                    tag_definitions={
                        k: TagDefinition(**v) for k, v in data["tag_definitions"].items()
                    },
                ...
                self.logger.error(f"Failed to load ontology version from {file_path}: {e}")
    ...
    def create_version(self, version: SemanticVersion, tag_definitions: Dict[str, TagDefinition], 
                      changelog: str, status: VersionStatus = VersionStatus.ACTIVE) -> bool:
        """Create a new ontology version."""
        # Generate hash signature
        content = json.dumps({k: asdict(v) for k, v in tag_definitions.items()}, sort_keys=True)
        ...
        if status == VersionStatus.ACTIVE:
            for existing_version in self.versions.values():
                if existing_version.status == VersionStatus.ACTIVE and existing_version.version < version:
    ...
    def deprecate_tag(self, tag_name: str, replacement_tag: Optional[str] = None) -> bool:
    ...
    def get_version_history(self) -> List[Dict[str, Any]]:
        ...
        history = []
        for version_str, version_obj in sorted(self.versions.items(), 
