...
class PolicyEvaluator:
    ...
    def run_ab_test(self, 
                   ...
                   evaluation_name: str = None) -> ComparisonResult:
        """
        ...
        self.logger.info(f"Starting A/B test {evaluation_id}: {policy_a.name} vs {policy_b.name}")
        ...
        outcome_deltas = self._compute_outcome_deltas(policy_a_outcomes, policy_b_outcomes)
    ...
    def _run_policy_on_drive(self, 
                           policy: PolicyConfig, 
                           drive: DriveContext, 
                           policy_function: Callable) -> PolicyOutcome:
        ...
        try:
            ...
            for play_idx, historical_play in enumerate(drive.historical_plays):
                try:
                    ...
                    current_situation = self._update_situation(current_situation, play_result)
            ...
            final_outcome = self._determine_drive_outcome(plays_executed, drive.actual_outcome)
            
            # Compute metrics
            metrics = self._compute_drive_metrics(plays_executed, final_outcome, drive.actual_outcome)
    ...
    def _default_policy_action(self, policy: PolicyConfig, situation: GameSituation) -> str:
    ...
    def _simulate_play_outcome(self, action: str, situation: GameSituation) -> Dict[str, Any]:
        ...
        return {
            ...
            "success": yards_gained >= situation.distance if situation.down == 3 else yards_gained > 0,
            "drive_ended": False,  # Simplified
            "turnover": random.random() < 0.02,  # 2% chance
        }
    
    def _update_situation(self, situation: GameSituation, play_result: Dict[str, Any]) -> GameSituation:
    ...
    def _determine_drive_outcome(self, plays: List[Dict[str, Any]], actual_outcome: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _compute_drive_metrics(self, plays: List[Dict[str, Any]], 
    ...
    def _compute_outcome_deltas(self, outcomes_a: List[PolicyOutcome], 
                               outcomes_b: List[PolicyOutcome]) -> Dict[str, float]:
    ...
    def _compute_statistical_significance(self, outcomes_a: List[PolicyOutcome], 
                                        outcomes_b: List[PolicyOutcome]) -> Dict[str, float]:
    ...
    def _generate_comparison_summary(self, policy_a: PolicyConfig, policy_b: PolicyConfig,
        ...
        if points_delta > 0:
            summary_parts.append(f"{policy_a.name} scored {points_delta:.2f} more points per drive")
        elif points_delta < 0:
            summary_parts.append(f"{policy_b.name} scored {abs(points_delta):.2f} more points per drive")
        else:
        ...
        if abs(yards_delta) > 1:
            if yards_delta > 0:
                summary_parts.append(f"{policy_a.name} gained {yards_delta:.1f} more yards per drive")
            else:
                summary_parts.append(f"{policy_b.name} gained {abs(yards_delta):.1f} more yards per drive")
        ...
        for evaluation in self.evaluations:
            if evaluation.evaluation_id == evaluation_id:
                return {
                    ...
                    "detailed_outcomes": [asdict(outcome) for outcome in 
                                        evaluation.policy_a_outcomes + evaluation.policy_b_outcomes]
                }
        return {}